...
class upload
{
	...
	var $file_dir="uploadfile/upfiles";// 上传的文件夹
	...
	function setData($data = array()){
		...
		$this->file_dir = isset($data['file_dir'])?$data['file_dir']:"uploadfile/upfiles/images/";//设置图片上传的路径
		...
		$this->siteid = isset($data['siteid'])?$data['siteid']:"";//截图类型
	...
	*/
	...
	function upfile($data = array()){
		...
		$mysql=pc_base::load_model('upfiles_model');
		$error = "";
		...
		$this->setData($data);
		$newDir = PHPCMS_PATH.$this->file_dir;
		//上次多个图片
		...
		if (is_array($_FILES[$file]['name'])){
			...
			foreach($_FILES[$file]['name'] as $i => $value){
				if ($value!=""){
					...
					//$thumb_file=SITE_PROTOCOL.SITE_URL.'/'.$this->file_dir.$newFile;
					watermark($allFile,$allFile,$this->siteid);
					//thumb($thumb_file);
					$this->_output();
	...
	function litpic ($data = array()){
		...
		$this->setData($data);
		$this->setSrcImg(PHPCMS_PATH.$data['url']);//设置图片生成路径
		$this->setCutimg();//截取的类型
		$this->setDstImg(PHPCMS_PATH.$data['new_url']);
		$this->_createMask();
		$this->_output();
	}
  ...
  /**
    ...
    function _loadMaskImg(){
        ...
        $src = '';
        if(function_exists("file_get_contents")){
            $src = file_get_contents(PHPCMS_PATH.$this->mask_img);
        } else {
