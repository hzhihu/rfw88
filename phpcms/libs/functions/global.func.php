...
function my_error_handler ($errno, $errstr, $errfile, $errline)
{
    ...
    if (pc_base::load_config('system', 'errorlog')) {
        ...
        $str = '<div style="font-size:12px;text-align:left; border-bottom:1px solid #9cc9e0; border-right:1px solid #9cc9e0;padding:1px 4px;color:#000000;font-family:Arial, Helvetica,sans-serif;"><span>errorno:' .
                 $errno .
                 ',str:' .
                 $errstr .
                 ...
                 '</font>,line' .
                 $errline .
                 '<br /><a href="http://faq.phpcms.cn/?type=file&errno=' .
                 $errno .
                 '&errstr=' .
                 urlencode($errstr) .
                 '&errfile=' .
...
function menu_linkage ($linkageid = 0, $id = 'linkid', $defaultvalue = 0)
{
    ...
    if ($datas ['style'] == '1') {
        ...
        if (! defined('DIALOG_INIT_1')) {
            ...
            // TODO $string .= '<link href="'.CSS_PATH.'dialog.css"
 ...
 *            引入外部字体时，字体大小
 * @param string $font
 *            字体名称 字体请放于phpcms\libs\data\font下
...
function p_template ($plugin = 'content', $template = 'index', $style = 'default')
{
    ...
    if (! file_exists($compiledtplfile) ||
             ...
             '.html') >
             filemtime($compiledtplfile))) {
        $template_cache->template_compile('plugin/' .
