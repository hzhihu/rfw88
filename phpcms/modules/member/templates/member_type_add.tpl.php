...
$(function(){
	...
	$("#name").formValidator({onshow:"<?php echo L('input').L('TYPE_NAME')?>",onfocus:"<?php echo L('TYPE_NAME').L('between_2_to_8')?>"}).inputValidator({min:2,max:15,onerror:"<?php echo L('TYPE_NAME').L('between_2_to_8')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"m=member&c=member_type&a=public_checkname_ajax",
		datatype : "html",
		async:'false',
...
<form name="myform" action="?m=member&c=member_type&a=add" method="post" id="myform">
<fieldset>
	...
	<table width="100%" class="table_form">
		...
		<tr>
			...
			<td><input type="text" name="info[desc]" class="input-text" id="desc" value=""></input></td>
		</tr>
		<tr>
			<td><?php echo L('TYPE_VIEW')?></td> 
			<td>
			<input type="radio" name="info[disable]" id="disable_yes" value="1" checked><label for="disable_yes"><?php echo L('TYPE_YES');?></label>
			<input type="radio" name="info[disable]" id="disable_no" value="0"><label for="disable_no"><?php echo L('TYPE_NO');?></label>
			</td>
		</tr>
	</table>
</fieldset>
